...
long long exponentiation(long long base_num, long long power);


int main()
{
    std::cout << pow(1000,1000);
    int num1, num2;
    std::cout << "Exponentiation: \n";
    std::cout << "Enter Base: ";
    std::cin >> num1;
    num2 = 1;
    std::string test_result = "Correct!";
    while(test_result == "Correct!")
    {
        long long exp_result = exponentiation(num1, num2);
        std::cout << num2;
        std::cout << "\nresult: " << exp_result << std::endl;
        if(exp_result != pow(num1,num2))
            test_result = "False!";
        else
            num2++;
    }
...
long long exponentiation(long long base_num, long long power)
{
    ...
    if(power % 2 == 0)
    {
       //create temp variable so you don't need to run alg twice
       std::string temp = std::to_string(exponentiation(base_num, power/2));
       return karatsuba(temp,temp);
    }
    else
    {
        std::string temp = std::to_string(exponentiation(base_num, power/2));
        long int temp2 = karatsuba(temp,temp);
        return karatsuba(std::to_string(base_num),std::to_string(temp2));
    }
}
